...
public class SemiprimeEvaluator {
	...
	/**
	 * Creates CSV data based on an Excel template.
	 * Prime, Prefix, Allowed?, k partitions, partitions (n fields)
	 ...
	 */
	public static List<String> prepareCsvFile(List<Integer> primes, Map<Integer, Sequence> semiprimes) {
		...
		for (Integer prime : primes) {
			if (semiprimes.containsKey(prime)) {
				...
				lines.add( semiprimeLine(prime, maxSequenceLength, semiprimes.get(prime), semiprimeAllowed) );
			} else {
	...
	private static String semiprimeLine(int prime, int maxSequenceLength, Sequence sequence, boolean allowed) {
		String line = prime + "," 
			+ sequence.getSign() + ","
			+ (allowed ? "1" : "0") + ","
			+ (sequence.getFactors().size()-2) + ",";
		for (Integer factor : sequence.getFactors()) {
			line += factor + ",";
		}
		return line + repeatString(",", maxSequenceLength - sequence.getFactors().size());
	}
	...
	private static Map<Integer, Boolean> constructAllowedList(Map<Integer, Sequence> semiprimes) {
		Map<Integer, Boolean> allowed = new HashMap<Integer, Boolean>(semiprimes.size());
		allowed.put(2, true);
		allowed.put(3, true);
